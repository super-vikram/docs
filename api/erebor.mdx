...
description: "Complete reference for the Erebor Go management API — ~89 endpoints across auth, vaults, analytics, merkle, registry, and more."
---
...
Erebor is the primary backend for SuperVaults management. It is a Go service exposing ~89 REST endpoints organized into 14 domains.
...
<Note>
The Erebor OpenAPI spec does not currently include the Zap Terminal AI analysis endpoint (`GET /api/v1/vaults/{vaultAddress}/ai/analysis?chain_id={}`). This endpoint is operational but undocumented in the spec.
</Note>
...
| `/api/v1/auth/verify` | GET | JWT | JWT verification. Returns `{ verified, walletAddress, isManager, vaults[] }`. |
...
| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/api/v1/public/vaults` | GET | None | Public list of all vaults with TVL and APY |
| `/api/v1/vaults` | GET | JWT | Authenticated vault list (`?detail=minimal&limit=500`) |
| `/api/v1/vaults/{chain_id}/{vault_address}` | GET | JWT | Full vault detail — PPS, supply, allocation, managers |
| `/api/v1/super-governor-config/{chain_id}` | GET | JWT | Protocol-level constraints for vault creation |

---

## Analytics (16 Endpoints)

All analytics endpoints are under `/api/v1/vaults/{chain_id}/{vault_address}/analytics/`:
...
| `risk/concentration` | GET | Concentration breakdown (not surfaced in UI yet) |
| `risk/drawdown` | GET | Drawdown detail (not surfaced in UI yet) |
| `risk/volatility` | GET | Volatility detail (not surfaced in UI yet) |
| `users` | GET | Total/active/new/churned users |
| `users/retention` | GET | Retention cohorts |
| `users/top` | GET | Top holders by balance |
| `users/cohorts` | GET | Cohort analysis (not surfaced in UI yet) |
| `operations` | GET | Tx count, gas, slippage |
| `operations/transactions` | GET | Transaction history table |
| `operations/volume` | GET | Deposit/withdraw volume |
| `fees?limit=200` | GET | Fee history and skim events |
| `social` | GET | Social feed for Zap Terminal |
| `export` | GET | Analytics export (not surfaced in UI yet) |

**Note:** `POST /api/v1/analytics/compare` for cross-vault comparison is available but not surfaced in the UI.
...
| `/api/v1/merkle/trees/{id}` | GET | Single tree (not surfaced in UI yet) |
| `/api/v1/merkle/proofs` | GET | All leaf proofs |
| `/api/v1/merkle/proofs/lookup` | GET | Single proof lookup (not surfaced in UI yet) |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/yield-sources/{chain_id}/{vault_address}` | GET | Whitelisted yield sources for vault |
...
## Vault Settings (4 Endpoints)

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/vaults/{chain_id}/{vault}/redeem-timelock` | GET | Current timelock (404 = none) |
| `/api/v1/vaults/{chain_id}/{vault}/redeem-timelock` | PUT | Set/update timelock |
| `/api/v1/vaults/{chain_id}/{vault}/redeem-timelock` | DELETE | Remove timelock |
| `/api/v1/redeem-timelocks/bulk` | PUT | Bulk set across vaults |
| `/api/v1/redeem-timelocks` | GET | Bulk read (not surfaced in UI) |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/vaults/create/prepare` | POST | Get vault creation calldata |
...
| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/audit/logs` | GET | Query audit trail (not surfaced in UI yet) |
| `/api/v1/audit/logs/{id}` | GET | Single audit log entry |
| `/api/v1/audit/export` | GET | Export audit logs |

Audit logs are available for automation — see [Webhooks & Event Streaming](/automation/webhooks).

---

## Undocumented Endpoint: Zap Terminal AI

The following endpoint is called by the Zap Terminal in the Dashboard but is not included in the Erebor OpenAPI spec:
...
Use with caution — the schema and stability of this endpoint are not officially documented. The Erebor spec will be updated to include this endpoint in a future release.
