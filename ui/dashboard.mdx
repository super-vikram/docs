...
Select your chain and vault from the top dropdowns. Financial data refreshes every 15 seconds from `GET /api/v1/vaults/{chain_id}/{vault_address}` plus 16 Erebor analytics sub-endpoints.
...
Fee history table: paginated (last 200 events) from `analytics/fees?limit=200`.
