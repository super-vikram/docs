...
SuperVaults is a polling-first monitoring model until the webhook system is implemented. Use these intervals as a baseline:
...
def check_pps_staleness(vault):
    ...
    if remaining < 3600:  # <1 hour remaining
...
Monitor `pending` redemption executions accumulating without `success` transitions:
...
def alert(severity: str, message: str):
    ...
    # e.g.: send_slack_message(f"*{severity}*: {message}")
...
function alert(severity: string, message: string) {
  ...
  // Replace with: sendSlack(message), pagerdutyAlert(message), etc.
}
...
Replace the `alert()` stub in the examples above with your notification system:
