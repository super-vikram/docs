...
🌐 Public · 🟡 Partial — Audit Logs Available Now, Webhooks Planned

<Warning>
The **webhook system is not yet implemented**. The information below covers what is available today (audit logs + polling) and documents the planned webhook schema for early integrators building against the future API.
</Warning>

## Current: Audit Logs as Event Stream

The audit log endpoint is available now in Erebor and serves as an interim event stream for vault operations:
...
When the webhook system ships, it will provide push-based event delivery to eliminate polling.
...
### Planned Webhook Delivery Format
...
- **Slack** — post to a channel on event trigger
- **PagerDuty** — page on-call for critical events
- **Discord** — post to a server channel
- **Custom HTTP** — POST to any HTTPS endpoint

### Planned Retry Policy

Failed deliveries would be retried with exponential backoff (1s → 2s → 4s → 8s → 16s, max 5 attempts).

---

## Current Workaround Summary

Until webhooks are implemented:
...
See [Monitoring Vault State](/automation/monitoring) for production-ready polling examples.
